...
const ChatScreen = ({ route }) => {
  ...
  const [inputTool, setInputTool] = useState(false);
    ...
    try {
      ...
      setMessages((prev) => {
        ...
        const updatedMessages =
          pageNumber === 1 ? uniqueMessages : [...prev, ...uniqueMessages];
  ...
  useFocusEffect(
    useCallback(() => {
        ...
        onEvent: (data) => {
          ...
          if (
            ...
            if (msgObj.attachment && msgObj.attachment.file) {
              ...
              const ext =
                msgObj.attachment.title?.split(".").pop().toLowerCase() || "";
              ...
              if (type === "image") {
                ...
                if (["mp4", "mov", "avi", "webm"].includes(ext)) {
                  ...
                  newMsg.text = `📎 ${
                    msgObj.attachment.title || "File attached"
                  }`;
                  newMsg.file = fileUrl;
                }
              }
      ...
      subscribeChannel({
        channelName,
        eventName: "messageSeen",
        onEvent: (data) => {
          console.log("👀 Message seen event aya:", data);

          setMessages((prev) =>
            prev.map((m) => {
              const isMyMessage = String(m.user._id) === String(currentUserId);
              // console.log('m.id', m.id);
              // console.log('data.message_id', data.message_id)
              // const isSameMessage = String(m._id) === String(data.message_id);

              // console.log("isMyMessage:", isMyMessage, "isSameMessage:", isSameMessage);

              if (isMyMessage) {
                console.log("✅ Seen update:", m._id);
                return {
                  ...m,
                  status: "seen",
  ...
  const sendMessageToApi = async ({ text, fileUri, fileName, type }) => {
    ...
    try {
      ...
      if (type === "video") {
        ...
        uploadUri = await VideoCompressor.compress(fileUri, {
      ...
      if (type === "image") {
        ...
        ext = fileName?.endsWith(".wav")
          ? "wav"
          : Platform.OS === "ios"
          ? "m4a"
          : "mp3";
        mimeType = fileName?.endsWith(".wav")
          ? "audio/wav"
          : Platform.OS === "ios"
          ? "audio/mp4"
          : "audio/mpeg";
      ...
      if (type === "image" && previewMsg.image)
        newMsg.image = serverRes.attachment
          ? JSON.parse(serverRes.attachment).new_name
          : previewMsg.image;
      if (type === "video" && previewMsg.video)
        newMsg.video = serverRes.attachment
          ? JSON.parse(serverRes.attachment).new_name
          : previewMsg.video;
      if (type === "audio" && previewMsg.audio)
        newMsg.audio = serverRes.attachment
          ? JSON.parse(serverRes.attachment).new_name
          : previewMsg.audio;
  ...
  const renderCustomActions = (props) => {
    ...
    return (
      <>
        {/* <TouchableOpacity
          onPress={() => setInputTool((prev) => !prev)}
          style={styles.toggleButton}
          accessible={true}
          accessibilityLabel="Toggle chat tools"
        >
          <Ionicons
            name={inputTool ? "close" : "ellipsis-vertical"}
            size={24}
            color="#6C63FF"
          />
        </TouchableOpacity> */}
        ...
        {!isLoading && (
          <>
            ...
            {inputTool && (
              <Animated.View
                entering={FadeIn.springify().damping(15).mass(1)}
  ...
  const renderTicks = (message) => {
    // Sirf apne bheje gaye messages ke liye show karo

    // Seen time ko format karo
    ...
    switch (message.status) {
      case "seen":
        return (
          <View style={{ flexDirection: "row", alignItems: "center" }}>
            ...
            {/* {message.seen_at && (
            <Text style={{ marginLeft: 4, fontSize: 10, color: "gray" }}>
      ...
      case "delivered":
        return <Text style={{ color: "gray", fontSize: 12 }}>✓✓</Text>;
      case "pending":
        return <Text style={{ color: "gray", fontSize: 12 }}>⏱</Text>;
  ...
  const renderMessageImage = (props) => {
    ...
    const currentIndex = imageMessages.findIndex(
      (img) => img.uri === currentMessage.image
    );
    ...
    return (
      <View
        style={[
          styles.mediaWrapper,
          isCurrentUser ? styles.rightMediaWrapper : styles.leftMediaWrapper,
        ]}
      >
        <TouchableOpacity
          onPress={() => {
        ...
        <View
          style={[
            ...
            isCurrentUser
              ? styles.rightTickContainer
              : styles.leftTickContainer,
          ]}
        >
          ...
          {isCurrentUser && renderTicks(currentMessage)}
        </View>
      </View>
    );
  };
      ...
      return (
        ...
        >
          ...
          >
            <ActivityIndicator
              size="small"
              color={isCurrentUser ? "#FFF" : "#6C63FF"}
            />
            <Text
              style={[
                styles.messageText,
    ...
    if (currentMessage.video) {
      return (
        <MessageVideo
          ...
          isCurrentUser={isCurrentUser}
          renderTicks={isCurrentUser && renderTicks(currentMessage)}
        />
      );
    }
            ...
            onPress={() => {
              ...
              Alert.alert("File", "Would you like to download this file?", [
                { text: "Cancel", style: "cancel" },
                {
                  text: "Download",
                  onPress: () =>
                    console.log("Download file:", currentMessage.file),
                },
              ]);
            }}
          >
      ...
      return (
        <View style={styles.typingContainer}>
          <Text style={styles.typingText}>
            {userInfo?.name || "Someone"} is typing...
          </Text>
